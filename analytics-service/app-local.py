...
DYNAMODB_ENDPOINT = os.getenv("DYNAMODB_ENDPOINT")  # Nova variável
...
try:
    ...
    dynamodb_client = session.client(
        "dynamodb",
        endpoint_url=DYNAMODB_ENDPOINT  # None = AWS real, URL = local
    )
    
    if DYNAMODB_ENDPOINT:
        log.info(f"DynamoDB conectado em: {DYNAMODB_ENDPOINT}")
    else:
        log.info(f"DynamoDB conectado na AWS região {AWS_REGION}")
